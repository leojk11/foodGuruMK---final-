$(document).ready(function(){
	...
	// search meals or restauratns
	$('.search_input').on('keypress', async e => {
		if(e.which == 13){
			const searched = $('.search_input').val();
			// console.log(searched);

			$.ajax({
				url: 'client/search?searched=' + searched,
				success: function(response){
					const allMealsDiv = $('#all_meals');
					const meals = response.searchResult;

					if(meals.length > 0){
						allMealsDiv.html('')
						for(var i = 0; i < meals.length; i++) {

							const homeMealWrapper = $('<a>').addClass('meal_snippet_wrapper').attr({ 'id': meals[i].ID });
							const mealSnippet = $('<div>').addClass('meal_snippet');
								const mealTop = $('<div>').addClass('meal_top');
									const mealName = $('<div>').addClass('meal_title').append(meals[i].Name);
								mealTop.append(mealName);
							mealSnippet.append(mealTop);
			
								const mealImage = $('<div>').addClass('meal_image');
									const image = $('<img>').attr({ 'src':'/superadmin/getMealImage?image=' + meals[i].Image }).css({  'width':'100%', 'height':'200px', 'object-fit':'cover' });
							...
							mealSnippet.append(mealImage)
								...
								mealExtraInfo.append(' ' + meals[i].Restaurant_name);						
							mealSnippet.append(mealExtraInfo);
			
								const mealPrice = $('<div>').addClass('meal_snippet_price').append(meals[i].Price + ' ' + 'ДЕН');
							mealSnippet.append(mealPrice);
									 
			
							homeMealWrapper.append(mealSnippet)
			
							allMealsDiv.append(homeMealWrapper);
						};	
